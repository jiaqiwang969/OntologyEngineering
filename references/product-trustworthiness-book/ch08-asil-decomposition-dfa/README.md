...
正文见 [chapter.md](chapter.md)。本章保留分解方案、共存判据、依赖失效与独立性证据的可读论述、术语、方法和
合成教学案例；机器可执行语义不在本目录。

## 唯一执行绑定

- package id：`semantica.chapter_packages.vol2.ch08`
- primary scenario：`semantica.vol2.ch08.scenario.primary`
- manifest 状态：`partial`
- release 状态：`blocked`
...
semantica package show semantica.chapter_packages.vol2.ch08 --json
semantica package run semantica.chapter_packages.vol2.ch08 \
...
`blocked`、未运行检查或 unsupported 能力改写成通过。未发现依赖不等于证明独立，未知项必须保留。
