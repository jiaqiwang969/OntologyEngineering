...
每张卡固定五个部分：**标题行**（表号与主题）；**溯源行**（要求条款、原文表号与 1-based 物理页、行数与格数）；**推荐矩阵**（按合法持有的受控来源人工核对录入的方法 × ASIL 全矩阵，含适用前提脚注）；**一句话读法**（这张表的主旋律）；**易错提示**（本书提示需要重点辨析的格子）。溯源行的条款号格式为“Part-条款”，如 4-7.4.2.2.2 即 ISO 26262-4:2018 §7.4.2.2.2。这里的“核对录入”是出版校订事实，不表示当前已有可执行全表包。
...
**易错提示**：矩阵相同不等于证据对象相同：表的对象从架构设计换成了单元设计。项目可以复用同一表示策略，但应能说明它如何覆盖单元设计要求。未来全表包必须用类似 `recTable` 的显式表归属区分两张表；当前没有可执行全表门禁，更不会替项目作出选择。
...
当前**没有**覆盖 28 张表的 released Semantica package，也没有可执行的全表转录门禁。历史旧仓门禁曾比较 RDF 与 MinerU 表格提取件的部分已编码字段，但那是迁移来源证据，不是现行执行能力；它也从未证明本附录 Markdown 逐格相同，更不能证明任何项目决定正当。人工卡片需要分别处理三类判断并留痕：
...
`MethodTableSet → MethodTable → MethodRecommendation` 保存表、方法、ASIL 与推荐度，
但必须作为 Semantica package 的显式 delta 建立，不能从旧路径临时加载。

历史转录曾结合原 PDF、`pdftotext -layout` 与 MinerU 表格复核；识别损坏处（如 Part 6
...
候选模型中的 `recTable` 属性值得单独一提：同名方法（如“故障注入测试”）在不同表
可以是不同 `VerificationMethod` 个体，每个推荐单元再指向所属表。未来门禁可据此
