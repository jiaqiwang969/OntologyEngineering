...
CLOSEUPS = [("deck_side", NXOpen.View.Canned.Right, (-2800.0, 0.0, 1200.0), 3.0),
            ("deck_top", NXOpen.View.Canned.Top, (-2800.0, 0.0, 1200.0), 3.0),
            ("base_side", NXOpen.View.Canned.Right, (-2800.0, 0.0, 300.0), 3.0),
            ("infeed_top", NXOpen.View.Canned.Top, (-4300.0, 0.0, 1200.0), 3.0)]
...
def render_set(disp, tag, cams=None):
    ...
    try:
        ...
        pass
    out = {}
    for name, canned, centre, zoom in CLOSEUPS:
        view.Orient(canned, NXOpen.View.ScaleAdjustment.Fit)
        if cams is None:
            scale = view.Scale * zoom
            out[name] = scale
        else:
            scale = cams[name]
        try:
            view.SetOrigin(NXOpen.Point3d(*centre))   # absolute model point shown at the view centre
            view.SetScale(scale)                       # identical scale for original and copy
        except Exception as e:
            log("closeup cam %s failed: %s" % (name, str(e)[:100]))
        log("%s cam scale %.4f centre %s" % (name, scale, centre))
