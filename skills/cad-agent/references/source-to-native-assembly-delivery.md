...
A0-A8 authoring lane
  ...
  native CAD occurrence construction, persistence, and native readback
...
- [A6 - Author in native CAD as a resumable transaction](#a6---author-in-native-cad-as-a-resumable-transaction)
...
source artifact -> reusable definition -> occurrence path -> world transform
                ...
                -> native CAD definition -> native CAD occurrence -> persisted data identity
```
...
- Treat virtual definitions, suppressed items, purchased modules, and harness
  ...
  standalone B-Rep.
- Reconcile source and native CAD trees at occurrence identity level. A count
  subtraction is a discrepancy, not an explanation.

Use one `occurrence_reconciliation.rows[]` record for every source and native CAD
occurrence. Every side also carries its definition identity:
`source_occurrence_id` binds `source_definition_id`, and
`native_occurrence_id` binds `native_definition_id`. `MATCHED` requires all
four identities, the native CAD definition's provenance must bind that same source
definition, and the completeness row must name that exact native CAD definition.
Source-only exclusions and native CAD-only additions use a null identity and null
...
native CAD sets must equal their native readback totals. A mismatch may pass only
when every difference has such a disposition; a zero source or native CAD
...
Create one `geometry_definitions[]` record per delivered native CAD definition,
...
`placement_contracts[]` record. Record its native CAD and source-lineage identity,
...
Declare one evidence-backed `placement_root` anchor for the native CAD root
...
## A6 - Author in native CAD as a resumable transaction

- Use the parent `cad-agent` direct NXOpen/Journal entrypoint and exactly one
  writer in a fresh task directory. No MCP or unrelated interactive session.
...
- Create/reuse definitions deterministically, then create occurrences with one
  verified transform application. Preserve source-to-native CAD lineage and avoid
...
- Add NX positioning constraints or explicit mechanism relations only from the interface and DOF
...
- Work in bounded batches with named checkpoints. After any timeout or
  ambiguous write, inspect the same job through the NX direct runner; never blind-retry.
...
- source-to-native CAD occurrence reconciliation and transform/lineage coverage;
...
The v3 native readback also records duplicate and orphan occurrence counts and
...
For a `PERSISTED_NATIVE_ASSEMBLY` claim, an unsaved document, unresolved data
...
inspect native CAD or interpret the content or engineering relevance of referenced
...
native CAD model plus a clean animation is still not manufacturing or physical
assembly approval.
...
- Fourier N1 demonstrates source-native file closure, saved versus expanded
  ...
  holds, and occurrence-level reconciliation with a partial native CAD translation.
...
## NX v3 persistence binding

Use `native_target.cad_system=SIEMENS_NX`, `project_id`, `output_directory` and
`root_model_path`. All former occurrence/definition/host fields use `native_*`.
`data_file_identity` is `sha256:<root PRT hash>`; `document_creation_id` is the
project-controlled stable model ID, not a transient NX tag or Fusion cloud ID.
`native_readback.persisted_files` contains every PRT in the saved dependency closure
with `path` (relative to manifest, or absolute) and `sha256`. First and second
readbacks require distinct `native_session_id` values binding PID/start/job evidence.
Paths and hashes must reference the actual saved artifacts; do not populate this
template with favorable counts or fabricated receipts. Use `--check-files`.

The historical v2 checker is retained as a compatibility arithmetic/integrity core;
v3 maps names and adds NX requirements without dropping its checks. It never calls
Fusion or Semantica. Supporting evidence content still needs native/engineering
review. Legacy v2 claims remain historical, not NX acceptance.
