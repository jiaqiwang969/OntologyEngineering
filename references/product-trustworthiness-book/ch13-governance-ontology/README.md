...
正文见 [chapter.md](chapter.md)。本章保留活动角色、组织关系、独立性距离、授权激活与失效的可读论述、术语、方法和
合成教学案例；机器可执行语义不在本目录。

## 唯一执行绑定

- package id：`semantica.chapter_packages.vol2.ch13`
- primary scenario：`semantica.vol2.ch13.scenario.primary`
- manifest 状态：`partial`
- release 状态：`blocked`
...
semantica package show semantica.chapter_packages.vol2.ch13 --json
semantica package run semantica.chapter_packages.vol2.ch13 \
...
`blocked`、未运行检查或 unsupported 能力改写成通过。机器可以检查授权结构，不能创造裁决权。
