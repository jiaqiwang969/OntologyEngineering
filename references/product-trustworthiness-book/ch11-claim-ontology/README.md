...
正文见 [chapter.md](chapter.md)。本章保留Claim、scope、evidence、counterevidence、decision 与 reopen的可读论述、术语、方法和
合成教学案例；机器可执行语义不在本目录。

## 唯一执行绑定

- package id：`semantica.chapter_packages.vol2.ch11`
- primary scenario：`semantica.vol2.ch11.scenario.primary`
- manifest 状态：`partial`
- release 状态：`blocked`
...
semantica package show semantica.chapter_packages.vol2.ch11 --json
semantica package run semantica.chapter_packages.vol2.ch11 \
...
`blocked`、未运行检查或 unsupported 能力改写成通过。正文中的形式化记法是教学投影；可执行定义只在 Semantica。
