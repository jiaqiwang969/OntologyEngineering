...
正文见 [chapter.md](chapter.md)。本章保留对象族、标识、记录、版本、等同证据与未知的可读论述、术语、方法和
合成教学案例；机器可执行语义不在本目录。

## 唯一执行绑定

- package id：`semantica.chapter_packages.vol2.ch12`
- primary scenario：`semantica.vol2.ch12.scenario.primary`
- manifest 状态：`partial`
- release 状态：`blocked`
...
runtime/.venv/bin/python scripts/semantic_engagement.py run \
  ...
  --scenario semantica.vol2.ch12.scenario.primary
...
`blocked`、未运行检查或 unsupported 能力改写成通过。相似度只是检索线索，不能授权现实记录合并。
