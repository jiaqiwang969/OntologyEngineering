...
METHONTOLOGY & METHONTOLOGY & 马德里理工提出的经典方法论：从规范说明到维护的全流程。 \\
...
知识权威源 & Authoritative Knowledge Source & 系统中裁决事实对错的唯一可信来源；本书主张由本体担任。 \\
...
Fuseki & Apache Jena Fuseki & Jena 配套的 SPARQL 服务器，HTTP 端点开箱即用。 \\
...
Jena & Apache Jena & Java 语义网框架：RDF 模型、SPARQL、规则推理与 TDB 存储。 \\
...
owlready2 & Owlready2 & 把 OWL 类映射成 Python 类的库，可一行代码调用推理器。 \\
...
pySHACL & pySHACL & Python 的 SHACL 校验器，命令行一键产出校验报告。 \\
...
rdflib & RDFLib & Python 最常用的 RDF 库：解析、序列化与内存 SPARQL。 \\
...
SPARQLWrapper & SPARQLWrapper & Python 访问远程 SPARQL 端点的轻量客户端库。 \\
