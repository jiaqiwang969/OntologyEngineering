...
\item 将全部声明 ontology 实际装入 Dataset，并用独立 fresh runtime 与不同装载顺序
      证明 canonical snapshot、quad count 和 Dataset SHA 一致；
\item 让每个 scenario receipt 绑定完整 package asset map，并确认它产生在完整 TBox
      装载之后，而不是只绑定本次 query/case 子集；
...
Dataset 与完整资产 receipt 闭包仍只证明可复算的 RDF 执行边界；它不自动证明 OWL DL
一致性、现实工程验收或 release。那些结论必须由各自声明的检查与有权决定提供。
...
若候选可能走到 release 或 promoted，project binding 必须从首次 proposal 起就包含完整
有序生命周期上限；本轮动作再由 fresh task 收窄，commit/promote 各由独立授权收窄。
已用短前缀登记的 proposed candidate 不能换 binding 续跑，只能保留为历史，再以非空
successor delta 建立新候选。
