...
正文见 [chapter.md](chapter.md)。本章保留快照、差异、证据绑定、影响集与逐项裁决的可读论述、术语、方法和
合成教学案例；机器可执行语义不在本目录。

## 唯一执行绑定

- package id：`semantica.chapter_packages.vol2.ch17`
- primary scenario：`semantica.vol2.ch17.scenario.primary`
- manifest 状态：`partial`
- release 状态：`blocked`
...
semantica package show semantica.chapter_packages.vol2.ch17 --json
semantica package run semantica.chapter_packages.vol2.ch17 \
...
`blocked`、未运行检查或 unsupported 能力改写成通过。未登记变化不得被推断成无影响，旧绿不能静默沿用。
