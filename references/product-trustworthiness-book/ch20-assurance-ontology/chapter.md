...
![一册持续跳动的可信案卷把最后的决定与签名位置留给人](../handbook/figures-imagegen/art-ch20-v01.jpg "chapter-art")
...
结构，留在自家本体里，经显式对齐合同按需去查。**放行需要的不是一张吞下一切的大图，而是每个本体守好自家边界之后，愿意向桌面交出的那层薄接口。**十个本体
还是十个，桌面上多出来的只是一层。
...
加纸。真正的第三条路今天才走通：**“同一基线”不再是一条值得表扬的纪律，而是每份证据自带、门禁可核对、装包那一刻就能拒绝的属性——对表不再依赖谁恰好没睡。**
墙上第一句话，销账。
...
这句话切开了——被吃掉的和留下来的，第一次分得清清楚楚。**提示词与封装是围着模型裁的衣服，模型一换就得重裁；本体、事实、门禁与评测是自己家的骨头——模型更新吃掉的只有衣服，吃不掉骨头。**吴工后来自己补了结案陈词：“拐杖是给旧模型
当的，没错。骨头不是。”
...
![摊开案卷中顶层主张、中层论证结和底部证据卡构成动态案例，两张证据卡翘起并举起琥珀色过期旗，重开条件丝带绕向页边，右下签名框上方仍悬着一支笔。](../handbook/figures-imagegen/ch20-fig01-live-assurance-case-v01.jpg "活的保证案例会在对象、配置、证据或假设改变时主动暴露过期和重开条件。自动检查负责举旗，最终承担风险的决定和签名仍然属于人。")
...
取舍认不认，代价谁来担。**机器把案例算到“可签”，人把名字签成“担责”——这两步，谁也替不了谁。**
...
在 Semantica 的炼化状态里，这种区别被刻意拆开。`committed` 说明一份候选记忆已经
不可变地留下；`regression_passed` 说明旧问题与新反例没有被静默遗忘；
`release_complete` 说明 package、能力、receipt、来源和权利的技术条件闭合；只有另一次
有权授权才能把它 `promoted` 到行业 registry。即使如此，产品放行、书稿修改、代码 push
和对外 `published` 仍是各自独立的人的决定。六个生命周期状态不是六盏绿灯，
更不是一支自动签字的笔；它们只是
让签字者清楚自己正依据哪一版记忆、哪些证据和哪些未决项。

还有一层容易被误当作“流程配置”的保证边界：候选第一次登记时使用的 binding，是它
以后不可扩大的生命周期能力上限。若最初只允许走到 `proposed`，后来即使换上一份更宽
的 binding，也不能把同一条候选记录救成 `committed`；否则控制面就能在评审之后悄悄
改写当时的权限条件。正确做法是从首次 proposal 起声明完整的预期上限，再让每一次新的
task 只请求眼前动作，让 commit 与 promotion 分别引用新的有权决定。能力上限、当前请求
和人的授权三者分离，既不会让“有能力”自动变成“已获准”，也不会让一次短任务永久截断
一条本来准备接受完整保证审查的候选。

同样，来源锚点的哈希相等只证明“当时引用的字节仍是那些字节”，不能证明 successor
candidate 的新结论已经被本书吸收。每当闭包、receipt 或授权语义改变，影响记录都要重新
区分：正文原已覆盖的原则、新增的通用保证方法、只属于项目案例的数字，以及 ISO 推演
是否真的改变。没有这份新的内容绑定说明，旧 book-impact 记录只能作为历史来源证据，
不能充当“书稿已同步”的绿灯。
