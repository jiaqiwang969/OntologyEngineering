...
证据还不存在、需要比较候选或安排下一次验证时，先按
[决定、候选与取证活动](references/method-internalization.md)把主张接到必要功能和实现条件。
记录哪项缺口会改变选择、怎样取得证据及不同结果后的行动，再进入下列检查。
计划、活动执行、数据有效、主张得到支持和采用决定各自有范围与依据。
...
5. 沿主张依赖回写候选、CAD／工艺方案及验证计划。模型删掉必要界面时，只能撤回它对界面功能的支持；若源几何本身已有相反证据，另立有来源的挑战。参数、配置或原件变更后重新检查受影响的支持边，保留旧回执；通过原工作项的依赖重审相关费用、交期和输出，未查清的依赖保持未知。
