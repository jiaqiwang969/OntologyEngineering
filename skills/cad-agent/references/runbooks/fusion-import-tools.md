# cad-agent Fusion 作业工具族

把 2026-09-05/06 手搓出来的一次性驱动脚本产品化,使一次「大 STEP → Fusion 装配」
作业能够**无人值守**跑完:一次调用一个受保护 cell、每次调用留回执、可续跑、
传输安全按构造保证、失败分类后只重试真正可重试的那一类。

> 状态:**已安装**(2026-09-07 并入正本)。本文是 `~/.codex/skills/ontology-engineering/skills/cad-agent/scripts/`
> 里这一族工具的参考页;运行时安全规则的正本是 `../fusion-execution.md`
> (预算表、Peekaboo 模式、无人值守检查单、结果分类法都在那里),歧义分诊在
> `fusion-mcp-ambiguity-triage.md`。原始评审稿留在
> `proposed-2026-09-06-reliability/D-tools-README.md` 作为历史。
>
> **未闭合项**:`scripts/_common.py` 的 `classify_call()` / `RETRY_KINDS` 仍是本地占位,
> 尚未替换为 C 的 `fusion_ops.TAXONOMY`(其 docstring 也仍自称 LOCAL PLACEHOLDER)。
> 两套 kind 名并存;换算表在 `../fusion-execution.md` 的「作业工具族」一节。
...
| 工具 | 作用 | 是否会调用 Fusion |
|---|---|---|
| `fusion_preflight.py` | 作业前只读体检:端口归属、守卫 marker、Peekaboo 模式探测与推荐、窗口普查、电源断言、磁盘 | 否(`--health-probe` 默认 `tcp`,只连端口;可选 `http` 打普通 `GET /health`;`none` 完全不碰 socket。都不是 MCP 请求) |
...
2. **多行短字面量。** 生成的数组/字典一律 `py_json(..., indent=1)`,并且任何一行
   超过 `_common.MAX_SCRIPT_LINE = 400` 直接拒绝
   (通用调用路径 `fusion_ops.MAX_SCRIPT_LINE` 是 2000,更宽;两个值都是安装事实)。原因:一个 ~7 KB 的单行 `PARAMS`
...
## 作业工具族的 kind 与重试策略(非正本分类法)

`_common.classify_call()` 把一次 `fusion_call.py` 调用映射为下表。
注意这**不是** `fusion_ops.TAXONOMY`;两者的换算表在 `../fusion-execution.md`
的「作业工具族」一节,新代码一律用 `fusion_ops` 那一套。
...
```bash
SK=~/.codex/skills/ontology-engineering/skills/cad-agent/scripts
...
本工具族默认 `local`;注意 `scripts/fusion_call.py` 的默认是
`FUSION_CAD_PEEKABOO_MODE=local-ondemand`,两个入口不一致,完整说明见
`../fusion-execution.md` 的「Peekaboo 模式选择规则」。`auto` 走
`fusion_preflight.select_peekaboo_mode()`,
...
测试**没有随工具一起安装**到 skill 目录(本 skill 不带 `tests/`)。它们留在评审目录:

```bash
cd ~/153-天花板小车/ceiling_trolley_project/reports/fusion_reliability_20260906/D_tooling_peekaboo_doctor/proposed
...
- 预算表(cell / acceptance / subprocess 三层与 `--long-request`)、主机休眠纪律、
  Peekaboo 模式选择规则、无人值守检查单、结果分类法:`../fusion-execution.md`。
- 超时/歧义之后怎么办(恢复路径 A = `scripts/fusion_recovery.py` 的 AppLog 完成证据,
  路径 B = 换新 PID):`fusion-mcp-ambiguity-triage.md`。
- 事故史与强制检索表:`fusion-runtime-safety-incidents.md`。
- 调用方必须遵守的规则(含 `fusion_ops.py` 的用法):`fusion-mcp-caller-guide.md`。
