...
差异分析本身也不能直接改写行业记忆。一次任务结束时，系统先分别报告工程处置、
Semantica 的 execution/regression/receipt/release，以及学习判定。只有多个受控实践反复
暴露同一缺口，且现有 package 的 CQ、Shape 或规则确实辨认不出它，才形成完整 candidate。
候选要绑定本次快照与证据哈希，并带着旧 CQ、单因反例、歧义例和前版例进入治理外环；
未登记的变化不得被解释成“无影响”，没有新知识也不得靠空 commit 制造版本活动。
