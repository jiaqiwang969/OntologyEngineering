...
正文见 [chapter.md](chapter.md)。本章保留软件需求、架构、单元验证、工具链与证据保质期的可读论述、术语、方法和
合成教学案例；机器可执行语义不在本目录。

## 唯一执行绑定

- package id：`semantica.chapter_packages.vol2.ch07`
- primary scenario：`semantica.vol2.ch07.scenario.primary`
- manifest 状态：`partial`
- release 状态：`blocked`
...
semantica package show semantica.chapter_packages.vol2.ch07 --json
semantica package run semantica.chapter_packages.vol2.ch07 \
...
`blocked`、未运行检查或 unsupported 能力改写成通过。运行成功不证明软件安全，变化后旧证据必须重新裁决。
