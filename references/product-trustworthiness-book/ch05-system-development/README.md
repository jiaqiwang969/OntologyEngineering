...
正文见 [chapter.md](chapter.md)。本章保留FSR/TSR 派生、架构分配、接口、时间预算与集成验证的可读论述、术语、方法和
合成教学案例；机器可执行语义不在本目录。

## 唯一执行绑定

- package id：`semantica.chapter_packages.vol2.ch05`
- primary scenario：`semantica.vol2.ch05.scenario.primary`
- manifest 状态：`partial`
- release 状态：`blocked`
...
semantica package show semantica.chapter_packages.vol2.ch05 --json
semantica package run semantica.chapter_packages.vol2.ch05 \
...
`blocked`、未运行检查或 unsupported 能力改写成通过。闭合的教学追溯链不等于真实系统证据已经完整。
