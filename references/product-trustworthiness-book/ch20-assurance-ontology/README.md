...
正文见 [chapter.md](chapter.md)。本章保留跨证据绑定、快照、receipt、release verdict 与人的决定的可读论述、术语、方法和
合成教学案例；机器可执行语义不在本目录。

## 唯一执行绑定

- package id：`semantica.chapter_packages.vol2.ch20`
- primary scenario：`semantica.vol2.ch20.scenario.primary`
- manifest 状态：`partial`
- release 状态：`blocked`
...
`blocked`、未运行检查或 unsupported 能力改写成通过。机器可以形成可审查候选，不能替有权人接受风险或签字。
