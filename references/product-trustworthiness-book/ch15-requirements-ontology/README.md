...
正文见 [chapter.md](chapter.md)。本章保留需求完整性、派生理由、分配、接口与影响传播的可读论述、术语、方法和
合成教学案例；机器可执行语义不在本目录。

## 唯一执行绑定

- package id：`semantica.chapter_packages.vol2.ch15`
- primary scenario：`semantica.vol2.ch15.scenario.primary`
- manifest 状态：`partial`
- release 状态：`blocked`
...
semantica package show semantica.chapter_packages.vol2.ch15 --json
semantica package run semantica.chapter_packages.vol2.ch15 \
...
`blocked`、未运行检查或 unsupported 能力改写成通过。闭世界只对声明分母成立，不能证明现实依赖已记全。
