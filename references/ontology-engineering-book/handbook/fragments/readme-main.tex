...
Semantica commit、版本和 wheel SHA-256；读者不得手抄这些身份。先 \texttt{discover}，再按
\texttt{semantic-engagement-contract.md} 建立与所选
...
\codeline{\hspace*{2\ttcw}--binding\ /path/to/package-binding.json\ \textbackslash{}}
\codeline{\hspace*{2\ttcw}--task\ /path/to/task-envelope.json\ \textbackslash{}}
\codeline{\hspace*{2\ttcw}--scenario\ OE-V1-CH03-SCN-CQ-ACCEPTANCE-001}
\codeline{runtime/.venv/bin/python\ scripts/semantic\_engagement.py\ open\ \textbackslash{}}
\codeline{\hspace*{2\ttcw}--binding\ /path/to/workspace-binding.json\ \textbackslash{}}
\codeline{\hspace*{2\ttcw}--task\ /path/to/task-envelope.json\ \textbackslash{}}
\codeline{\hspace*{2\ttcw}--workspace\ /path/to/semantica-managed-registry}
\end{codebox}
\color{black}

\texttt{propose/commit/verify/history/promote} 仍调用 \texttt{scripts/semantic\_engagement.py}，并引用合同定义的
delta、candidate 与 regression/release evidence fixtures；先读对应子命令 \texttt{--help}。
原生 \texttt{semantica package list/show/run/verify/verify-books} 只用于底层 runner、manifest 与
