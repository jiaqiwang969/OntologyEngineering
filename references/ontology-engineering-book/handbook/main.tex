% 《工程本体论》· 2026.08 Semantica 唯一执行语义候选版
...
\begin{titlepage}
  ...
  \begin{tikzpicture}[remember picture,overlay]
    \node[inner sep=0pt] at (current page.center) {%
      \IfFileExists{figures/cover-series-v02-focus.jpg}{%
        \includegraphics[width=\paperwidth,height=\paperheight]{figures/cover-series-v02-focus.jpg}%
    ...
    \node[anchor=north west,inner sep=0pt,text width=8.75cm,align=left]
      at ([xshift=10.90cm,yshift=-1.52cm]current page.north west) {%
        ...
        {\fontsize{20}{27}\selectfont\heiti\color{seriesice}\mbox{Semantica作为唯一语义}}\par\vspace{0.28cm}
    ...
    \node[anchor=south west,inner sep=0pt,align=left,opacity=0.68]
      at ([xshift=1.59cm,yshift=0.84cm]current page.south west) {%
        ...
        {\fontsize{7.4}{9.8}\selectfont\songti\color{black}
          作者 claude F5 max · 5k · 用时 6h · 2026.08}\\[-0.04cm]
...
\textbf{版本说明}\quad 本册为《工程本体论》的 2026.08 Semantica 唯一执行语义候选版；作者署名为
...
\textbf{一体化说明}\quad 两卷书给出方法与可读规格，Semantica 承担全部可执行语义，
Skill 是默认语义接入与行业本体炼化控制面，而不只是检索入口。每次工程任务都要分开
返回工程结果、Semantica 结果与本体学习判定；场景 oracle 通过不自动等于 package
release 完成，更不等于经验已经晋升为行业知识。
...
\textbf{实践学习以同一 refinery 为准。}
工程任务先用带哈希证据的 task envelope 与 project binding 打开 source-locked semantic
engagement；结束时必须给出 \texttt{no\_delta} 及理由，或形成完整 package delta 候选。
候选依次经过 proposed、committed、regression passed、release complete 与 promoted，
每一步都保留独立状态和授权。书负责教人怎样判断，不能替项目作证，也不能替有权人
晋升、签字或发布。
