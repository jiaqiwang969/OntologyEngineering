...
> 唯一位置是 `semantica.chapter_packages.vol2.normative`，其中 Part 1 资产登记为
> `part1-vocabulary` 与 `part1-cards`。该 package 当前为 `partial`、release
> `blocked`；它保存的是本书发布的工程释义、来源坐标和教学映射，不是 ISO 原文、
...
1. **词条号**：形如 1-3.84，表示 ISO 26262-1:2018 第 3 章第 84 个词条。这是每个条目的受控身份，也是与原文核对的唯一入口。本附录定稿前曾用两种独立生成的提取视图交叉定位词条；两种视图来自同一份 PDF，不构成两个独立来源。其中四个词条（3.60、3.153、3.154、3.175）的标题行存在 OCR 噪声，已回看原 PDF 校订。
...
机器侧唯一对应物是 `semantica.chapter_packages.vol2.normative` 中的
`part1-vocabulary` 与 `part1-cards` 资产；它们为已迁移词条保存工程释义与来源坐标，
供该包声明的查询和门禁使用。**注意分母账**：Part 1 的受控词条集共 185 条；
package 当前是 `partial`、release `blocked`，必须从当次 manifest/receipt 读取实际覆盖，
...
- **其他技术（other technology）**（1-3.105 ｜ 首讲 ch02）——E/E 之外的技术，如机械、液压。标准不管它们的开发，但安全机制可以借助它们实现。
...
- **免于干扰（freedom from interference, FFI）**（1-3.65 ｜ 首讲 ch08）——两个及以上元素之间不存在会导致违背安全要求的**级联失效**。软件分区要证明的就是它：QM 分区不得拖垮 ASIL 分区。正文以“互不干扰”的叙事语汇出现，合规判据以本词条为准。
...
- **安全机制（safety mechanism）**（1-3.142 ｜ 首讲 ch02）——由 E/E 功能/元素或其他技术实现的**技术方案**，用于检测并缓解或容忍故障、控制或避免失效，以保持预期功能或达到安全状态。机制是措施的真子集：只有做进产品里的技术方案才叫机制，流程活动不算。
...
- **审查（inspection）**（1-3.82 ｜ 方法表见附录 D）——**遵循正式规程**检查工作产物以检出安全异常。与走查同为验证手段，区别在正式程度：审查有规程、角色和进出条件。
- **走查（walk-through）**（1-3.182 ｜ 首讲 ch08）——系统地过一遍工作产物以检出安全异常，形式比审查轻。方法表里两者是备选关系（Part 6 Table 7 的 1a/1c）。
...
Semantica normative package 的 `part1-vocabulary` 资产为**已迁移的术语子集**建立可
...
第二条三元组把 `Item` 类挂到词条锚点上。Semantica 场景可以检查这条绑定及已编码
坐标合同；至于 `Item` 的建模是否完整忠实于 1-3.84，仍须回到合法持有的原文和建模
...
当前自动检查的能力边界如下。`semantica.vol2.normative.scenario.modality-fidelity`
只执行其 CQ registry 明示的机器合同；坐标或绑定通过不表示 OCR、词条转述或原 PDF
已经完成语义核对，它也不读取本附录 Markdown，因而发现不了正文写错词条号。
`severity` 等 OCR 噪声仍须回看原 PDF 并以校订留痕处理，不能归功于绑定查询。
