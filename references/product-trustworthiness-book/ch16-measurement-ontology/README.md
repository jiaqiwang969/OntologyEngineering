...
正文见 [chapter.md](chapter.md)。本章保留被测对象、方法、单位、条件、不确定度、来源与范围的可读论述、术语、方法和
合成教学案例；机器可执行语义不在本目录。

## 唯一执行绑定

- package id：`semantica.chapter_packages.vol2.ch16`
- primary scenario：`semantica.vol2.ch16.scenario.primary`
- manifest 状态：`partial`
- release 状态：`blocked`
...
semantica package show semantica.chapter_packages.vol2.ch16 --json
semantica package run semantica.chapter_packages.vol2.ch16 \
...
`blocked`、未运行检查或 unsupported 能力改写成通过。数字可重算不等于数据真实、方法适用或证据充分。
