...
正文见 [chapter.md](chapter.md)。本章保留设计定义、批次、单件、事件、装车、维修与替代的可读论述、术语、方法和
合成教学案例；机器可执行语义不在本目录。

## 唯一执行绑定

- package id：`semantica.chapter_packages.vol2.ch19`
- primary scenario：`semantica.vol2.ch19.scenario.primary`
- manifest 状态：`partial`
- release 状态：`blocked`
...
semantica package show semantica.chapter_packages.vol2.ch19 --json
semantica package run semantica.chapter_packages.vol2.ch19 \
...
`blocked`、未运行检查或 unsupported 能力改写成通过。教学 ledger 与查询不构成真实产品谱系。
