...
首次试用 Jev 可选择[附带公开实验密钥的完整体验包](docs/JEV-TRIAL.md)，由所有者明确授权
提供共享临时凭据；安装运行时后由助手读取，撤销后可换成自己的密钥。

当前版本 **0.5.8** 增加共用工程证据方法、Jev 批量初判与持续制造协作，见[更新说明](docs/releases/manufacturing-method-0.5.8.md)。包含两卷书的完整版与体验包见[分发说明](docs/PORTABLE-DISTRIBUTION.md)。Jev 的独立质量和真实降本效果留待项目实践验证。
...
当前方法版本为 **0.5.8**，适用于离散机械制造。行业、材料、工艺参数与验收值由各项目
...
| 组件 | 当前版本与范围 |
|---|---|
| 制造方法 | `0.5.8`；通用 CAD 证据、形态/机构逆向重建、远程桥、装配/机构有界筛查、制造工艺与成本 |
