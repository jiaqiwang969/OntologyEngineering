...
正文见 [chapter.md](chapter.md)。本章保留特殊特性、配置、现场监控、维修与数字线程的可读论述、术语、方法和
合成教学案例；机器可执行语义不在本目录。

## 唯一执行绑定

- package id：`semantica.chapter_packages.vol2.ch09`
- primary scenario：`semantica.vol2.ch09.scenario.primary`
- manifest 状态：`partial`
- release 状态：`blocked`
...
semantica package show semantica.chapter_packages.vol2.ch09 --json
semantica package run semantica.chapter_packages.vol2.ch09 \
...
`blocked`、未运行检查或 unsupported 能力改写成通过。合成案例不能声明真实批次、单件、服务或现场事实。
