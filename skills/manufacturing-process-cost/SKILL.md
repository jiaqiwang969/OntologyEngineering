...
这是方法、脱敏案例和工作记录模板模块，已有发布方法版本为 `0.5.8`，本地方法修订与公开发布分别管理。记录模板版本以各文件为准；`discovery-record`、`solution-record`、`organization-run` 的 `0.3.0` 增加按需使用的需求来源、候选评价和验证活动关系，沿用原 ID 与记录职责。报告组件与图形模板仍为 `1.0.0`；六张评审卡为 `1.4.1`。CAD／工艺交接涵盖零件、图纸/BOM、装配、配合、机构状态与运动包络，再关联工艺功能和方案。工艺替代时的功能覆盖与反证回退继续适用。冻结的 Semantica 制造案例仍为 `0.1.1`；新增语义只可经受控后继候选验证，不能把本文当成已发布的语义门禁。先遵守根 [ontology-engineering](../../SKILL.md) 的 source lock、发现、绑定和三联输出规则；正式语义执行仍只走 Semantica。按 [语义执行与证据范围](references/semantic-use.md) 定位候选；可执行资产存于 Semantica 工作区，不在 skill 内另建语义正本。记录模板不是 `ProjectOntologyBinding`、`SemanticTaskEnvelope` 或语义校验器。缺少绑定只阻断依赖绑定的执行；继续已授权的资料整理、条件分析和问题准备。
...
大量对话、工艺建议与报价版本使用[共用批量判断](../../docs/judgment-intake.md)，沿用 CAD 的
对象身份与证据来源。独立记录需求、建议、观察和假设，同一段可兼有多种角色。
把必要功能、路线实现、检验依据和费用口径形成审查计划；功能覆盖由本次原生检查
汇总。路线或接口受挑战时，沿支持依赖重审检验、报价与交期，不另建制造专用判断器。
