...
contract_version: 1
chapter: ch06
executable_package_id: semantica.chapter_packages.vol2.ch06
...
source_anchors:
  ...
  - id: "10-Annex A"
    ...
    bbox: [389, 98, 551, 143]
planned_outputs:
  - references/product-trustworthiness-book/ch06-hardware-development/chapter.md
...
- 数字身份：`95.00%` 是加 U7 前 `Σλ=400 FIT` 合成基线；`95.48%` 是同一基线加入 3 FIT U7 后、在全部分类/时序/独立性假设成立时的重算；`99.4%` 是历史教学记录，Semantica ch06 当前尚未给它发布完成的计算工件绑定。
...
| 节 | 消费 | 产出与留问 |
|---|---|---|
| 6.1 三盏绿灯，为什么评审人不签字？ | 三条书稿教学记录、开场签字页 | 四项 Review Hold；先问分析范围 |
...
| 6.5 95% 和 99.4% 是同一个结论吗？ | 400 FIT 聚合、项目目标、书稿教学记录 | 数字身份账；再问 U7 的真实增量 |
...
| 6.8 本体化实践：怎样让 99.4% 变成可审的数字？ | Semantica ch06 package 的参考/项目/记录合同 | 当前能证与不能证的状态账；再问签字范围 |
| 6.9 评审最后能签什么？ | 全章产物 | `EPS-S06-HW` 候选与开放项；回到软件平行支路 |

## 保留与迁移

正文保留五类 λ、FTTI/MPFDTI、一行 FMEDA、400/403 FIT 手算、U7 自身故障、
Annex H 逻辑、Clause 8/9 边界和三层数值模型。唯一机器语义是 Semantica ch06
package；其迁移覆盖当前仍为 `partial`、release `blocked`。未进入 package contract、
scenario、oracle 与 receipt 的长表或教学细节只可作为人读材料，不能从书旁路径执行。

## 练习配置

练习围绕单因迁移：改 U7 检测时间、自检周期、独立性、失效率或教学达成值，要求
指出哪一笔信用撤回、哪个 Semantica 场景应拒绝，以及机器拒绝仍不能证明什么。
练习不以“列出十项知识点”代替迁移判断，也不在书稿中复制可执行约束。
...
手算 SPFM 95.00% 与 LFM 94.74%，再把参考目标、项目选择、合成计算与书稿教学记录
分开；99.4% 不得成为 95% 的修改后版本。
