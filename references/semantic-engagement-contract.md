...
| `ontology-engineering` Skill | 理解当前情景、组织能力协同、语义接入、学习编排和结果解释 | 第二套本体实现或后端 |
...
能力需求先由[默认 Jev 情景路由](context-routing.md#默认调用)提出候选，实际 skill
选择与执行由主 agent 读取并复核候选后统筹；调用失败或不能提供上下文时如实记录并继续。
Semantica 的 capability discovery 发现语义能力，
不等于发现了 CAD、求解器或任意外部 skill。概念解释和独立资料工作可先继续，
项目绑定只约束依赖它的正式执行。
...
接收工程任务
  → 续接情景，确定本轮产出、能力与交接依赖
  → 核对 Semantica source identity；正式执行需要时读取项目绑定
  → 从两卷书选择方法镜头和来源锚点
